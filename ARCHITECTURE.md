...
### Cliente
`Link por token OU placa + últimos 4 do telefone → permanência → tempo/valor → tarifa/serviços → QR → saída`
...
O token público da permanência é UUID aleatório. A consulta por placa não retorna o registro sem confirmação dos últimos 4 dígitos do telefone. O portal não expõe telefone, CPF ou dados administrativos.
