...
import { Ban, Building2, CheckCircle2, Clock3, RefreshCw, Search } from 'lucide-react';

type Plan = { id: string; name: string; duration_days: number; active: boolean };
type RawLicense = {
  plan_id: string;
...
type Organization = {
  ...
  name: string;
  slug: string | null;
  profiles?: Array<{ name: string; email: string | null; role: string }>;
...
export default function MasterLicencasPage() {
  ...
  useEffect(() => {
    void load().catch((reason) => setMessage(reason.message || 'Não foi possível carregar as licenças.'));
  }, [load]);
  ...
  const filtered = useMemo(
    () =>
      organizations.filter((organization) => {
        ...
        const term = `${organization.name} ${organization.slug || ''} ${owner?.name || ''} ${owner?.email || ''}`;
        return (filter === 'all' || status === filter) && term.toLowerCase().includes(query.toLowerCase());
      }),
  ...
  async function licenseAction(organizationId: string, action: string, planId?: string, reason = '') {
    ...
    setBusyId('');
    setMessage(response.ok ? 'Licença atualizada e propagada às sessões abertas.' : data.error);
    if (response.ok) await load();
  }

  return (
    <AppShell title="Licenças SaaS">
      <MasterNav />
      <div className="mb-6">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-brand-600">Controle comercial</p>
        <h1 className="mt-1 text-3xl font-black">Licenças</h1>
        <p className="mt-2 text-slate-500">
          Ative, renove ou bloqueie acessos sem misturar essas ações com o cadastro do estacionamento.
        </p>
      </div>

      <div className="grid gap-3 rounded-3xl bg-white p-4 shadow-sm dark:bg-slate-900 sm:grid-cols-[1fr_auto]">
        <label className="relative">
          <Search className="absolute left-3 top-3.5 text-slate-400" size={18} />
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Pesquisar estacionamento, proprietário ou e-mail"
        ...
        <select value={filter} onChange={(event) => setFilter(event.target.value)} className="master-input">
          ...
          <option value="pending">Sem licença</option>
...
function LicenseCard({
  ...
  useEffect(() => {
    setPlanId(license?.plan_id || plans.find((plan) => plan.active)?.id || '');
  }, [license?.plan_id, plans]);
  ...
  return (
    <article className="rounded-3xl bg-white p-5 shadow-sm dark:bg-slate-900">
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              {owner?.name || 'Sem proprietário'} • {owner?.email || 'Sem e-mail'}
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 grid gap-3 border-t border-slate-100 pt-4 dark:border-slate-800 lg:grid-cols-[1fr_1fr_auto]">
