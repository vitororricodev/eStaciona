...
export async function GET(req: NextRequest) {
  ...
  let query=supabase.from('stays').select('id,public_token,started_at,status,paid_at,paid_amount,payment_grace_until,vehicles!inner(id,plate,make,model,color,customers(id,name,phone)),tariff_plans(*),stay_services(id,service_name,unit_price,quantity)').eq('organization_id',profile.organization_id).eq('status','open');
