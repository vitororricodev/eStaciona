...
export async function GET(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const { data, error } = await supabase.from('stays')
    .select('public_token,started_at,ended_at,status,final_amount,paid_at,paid_amount,payment_grace_until,tariff_snapshot,vehicles(plate,make,model,color,customers(name)),tariff_plans(*),stay_services(service_name,unit_price,quantity)')
