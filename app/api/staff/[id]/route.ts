...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!profile || !isOwner(profile.role))
    return NextResponse.json({ error: 'Apenas proprietário pode alterar equipe.' }, { status: 403 });
  if (id === user?.id && p.data.active === false)
    return NextResponse.json({ error: 'Você não pode desativar seu próprio usuário.' }, { status: 409 });
