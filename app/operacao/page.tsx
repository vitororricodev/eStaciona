...
export default function OperacaoPage() {
  ...
  return <AppShell title="Operação"><div className="mx-auto max-w-2xl">
    ...
    <form onSubmit={search} className="mt-6 rounded-3xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-3"><Search className="text-slate-400"/><input value={plate} onChange={e=>setPlate(e.target.value.toUpperCase())} placeholder="Pesquisar placa para cobrar" className="w-full bg-transparent py-2 text-lg font-semibold uppercase outline-none"/><button className="rounded-xl bg-brand-50 px-4 py-2 text-sm font-black text-brand-700">Buscar</button></div></form>
